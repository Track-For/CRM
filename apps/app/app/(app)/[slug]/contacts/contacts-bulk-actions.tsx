...
export function ContactsBulkActions({
	...
	const setCompany = useMutation(
		trpc.contacts.bulkSetCompany.mutationOptions({
			onSuccess: async (result) => {
				await cache.contact();
				reportBulk(result, (count) => `Movido: ${contacts(count)}.`);
				onDone();
			},
			onError,
		}),
		...
		trpc.contacts.bulkEnrich.mutationOptions({
			onSuccess: async (result) => {
				...
				reportBulk(
					result,
					(count) =>
						`Buscando dados de ${contacts(count)} — a tabela vai atualizar.`,
				);
				onDone();
			},
			onError,
	...
	const archive = useMutation(
		trpc.contacts.bulkArchive.mutationOptions({
			onSuccess: async (result, variables) => {
				...
				reportBulk(result, (count) => `Arquivado: ${contacts(count)}.`);
				onDone();
			},
			onError,
		}),
	...
	const restore = useMutation(
		trpc.contacts.bulkRestore.mutationOptions({
			onSuccess: async (result) => {
				await cache.contact();
				reportBulk(result, (count) => `Restaurado: ${contacts(count)}.`);
				onDone();
			},
			onError,
		}),
	...
	const purge = useMutation(
		trpc.contacts.bulkPurge.mutationOptions({
			onSuccess: async (result, variables) => {
				...
				reportBulk(
					result,
					(count) => `Excluído para sempre: ${contacts(count)}.`,
				);
				setConfirming(false);
				onDone();
			},
		...
		return (
			<>
				...
				<BulkDeleteDialog
					...
					title={`Excluir ${contacts(ids.length)} para sempre?`}
					description="Os e-mails deles ficam bloqueados, então a sincronização da caixa de entrada não vai arquivá-los de novo. Isso não pode ser desfeito."
