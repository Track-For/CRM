		...
		<section className="flex flex-col gap-3 px-(--spacing-block-inline)">
			<div className="flex items-end justify-between gap-4">
				<div>
					<h2 className="font-medium text-sm">
						Canais que a Comp AI pode acessar
					</h2>
					<p className="text-muted-foreground text-xs">
						Os agentes escolhem a partir desta lista.
					</p>
				</div>
				<Button
					disabled={refreshing}
			...
			{channels.stalled ? (
				<p className="text-warning text-xs">
					A Comp AI não está lendo o Slack agora. A lista pode estar
					desatualizada.
				</p>
			) : null}
					...
					<p className="px-4 py-4 text-muted-foreground text-sm">
						{channels.pending
							...
							: query
								...
								: "Nenhum canal ainda. A Comp AI lê a lista do Slack depois de conectar."}
					</p>
				}
		...
		<AlertDialog open onOpenChange={(open) => !open && onCancel()}>
			<AlertDialogContent>
				<AlertDialogHeader>
					...
					<AlertDialogDescription>
						{canInviteItself
							? `É um canal privado, então a Comp AI entra como você. É como digitar o convite você mesmo. Todos no canal veem a entrada. Ela não lê nada até você ativar uma permissão.`
							: `Ainda não podemos adicionar a Comp AI a um canal privado. Alguém que já está em #${channel.name} precisa fazer isso.`}
					</AlertDialogDescription>
				</AlertDialogHeader>
