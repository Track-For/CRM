		...
		<div className="flex flex-col gap-4 px-(--spacing-block-inline)">
			...
			{matches.data.sync === "stalled" ? (
				<p className="text-center text-warning text-xs">
					A Comp AI não está lendo o Slack agora. Essas correspondências podem
			...
			<div className="flex flex-col divide-y border-y">
				{rows.length === 0 ? (
					<p className="py-5 text-center text-muted-foreground text-sm">
						Nenhum colega de equipe do CRM disponível para correspondência.
					</p>
				) : null}
				{rows.map((row) => (
					<div
			...
			<p className="text-muted-foreground text-xs leading-relaxed">
				Atualize depois que um e-mail do Slack mudar. O CRM só corresponde
				endereços de e-mail exatos. Quem não tiver correspondência exata fica
				sem correspondência, e um agente para em vez de adivinhar por um nome
				parecido.
			</p>
