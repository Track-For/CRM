...
const never = [
	...
	"Postar em qualquer lugar além do destino aprovado naquela automação",
...
const suggestions = [
	[
		...
		"Publica o negócio em um canal de vendas aprovado.",
	],
	[
		"Quando um negócio é ganho",
		"Avisa um canal aprovado que o negócio fechou.",
	],
	[
		"Quando um negócio reabre",
		"Notifica um canal ou colega de equipe aprovado.",
	],
];
		...
		<ConnectionPage centered className="max-w-(--container-page)">
			<header className="flex flex-col gap-3 px-(--spacing-block-inline)">
				...
				<p className="text-muted-foreground text-sm leading-relaxed">
					Conectar o Slack dá ao CRM uma forma de entrar e de sair. O que ele
					realmente faz com isso é decidido depois, uma automação de cada vez.
				</p>
			</header>
			<SlackScopeGroups
			...
			<div className="flex items-center gap-4 border-y px-(--spacing-block-inline) py-5">
				...
				<p className="text-muted-foreground text-xs">
					Você aprova o workspace no Slack. Você pode desconectá-lo aqui a
					qualquer momento.
				</p>
			</div>
			<section className="flex flex-col gap-3 px-(--spacing-block-inline)">
				<div>
					<h2 className="font-medium text-sm">
						Depois, a maioria dos times começa com um destes
					</h2>
					<p className="text-muted-foreground text-xs">
						Sugestões, não configurações. Nenhuma existe até você escolher uma e
						ativá-la.
					</p>
				</div>
		...
		<ConnectionPage>
			<header className="flex flex-col gap-2 px-(--spacing-block-inline)">
				...
				<p className="text-muted-foreground text-sm">
					{status.canManage
						? "Aqui está o que o Slack nos deu. Os agentes só postam onde a automação diz."
						: "Aqui está o que o Slack nos deu. Só um proprietário ou administrador pode desconectá-lo."}
				</p>
			</header>
			<MissingGrant missing={missing} slug={slug} />
			<SlackScopeGroups
				groups={groupScopes(status.scopes)}
				title="O que este workspace concedeu"
			...
			<section className="flex flex-col gap-3 border-y px-(--spacing-block-inline) py-5">
				...
				<div className="flex flex-col divide-y rounded-lg border">
					...
					<Link
						...
						href={`/${slug}/chat`}
					>
						Descrever outro agente no chat
					</Link>
				</div>
			</section>
			<div className="flex items-center justify-between gap-4 px-(--spacing-block-inline)">
				<p className="text-sm">
					{status.people.reviewed === 0
						? "Nenhuma pessoa do workspace foi revisada ainda."
						: `${status.people.matched} de ${status.people.reviewed} pessoas revisadas correspondem.`}
				</p>
			...
			<Alert variant="warning">
				...
				<AlertTitle>
					{privateChannels
						...
						: `O Slack negou ${missing.length} permiss${missing.length === 1 ? "ão" : "ões"}`}
				</AlertTitle>
				<AlertDescription>
					<span>Reconecte para pedir de novo. Você não perde nada.</span>
