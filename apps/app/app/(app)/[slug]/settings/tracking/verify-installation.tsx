	...
	return (
		<Card>
			<CardHeader>
				...
				<CardDescription>
					Carregamos uma página e procuramos o script, depois lemos seu
					container do Tag Manager se ele não estiver no HTML.
				</CardDescription>
			...
			<CardContent>
				...
				>
					<Field>
						...
						<FieldDescription>
							A página precisa ser pública. Uma página atrás de login sempre
							falha nessa verificação.
						</FieldDescription>
					</Field>
				</form>
...
function Indicator({ result }: { result: Result }) {
	...
	if (result.status === "found" && result.container?.carriesSiteId === false) {
		return (
			<StatusIndicator
				size="sm"
				tone="warning"
				label="O Tag Manager precisa de correção"
			/>
		);
	}
	...
	return (
		<StatusIndicator
			...
			tone="warning"
			label={
				result.status === "found" ? "Sem visualização ainda" : "Não detectado"
			}
		/>
	);
}
...
function Outcome({ result, siteId }: { result: Result; siteId: string }) {
			...
			<Alert variant="destructive">
				...
				<AlertDescription>
					...
					{result.containers.length > 0
						? ` Também lemos o container do Tag Manager ${result.containers.join(" e ")}, e a tag também não está lá.`
						: ""}
				</AlertDescription>
			</Alert>
		);
	...
	if (result.container && !result.container.carriesSiteId) {
		return (
			<Alert variant="destructive">
				...
				<AlertDescription>
					O container {result.container.id} carrega a tag, mas o ID do site não
					está na URL do script. O Tag Manager mantém apenas a URL ao injetar um
					script, então um atributo data-site nunca chega à página e o
					rastreador nunca inicia. Copie o trecho do Tag Manager acima e
	...
	return (
		<Alert>
			...
			<AlertTitle>
				{result.container
					? `Script encontrado no container ${result.container.id}`
			...
			<AlertDescription>
				...
				permissões.
				{result.container
					? " A tag não está no HTML, então ela só roda quando o Tag Manager a dispara — uma visualização de página é a prova."
					: ""}
				{result.pageView
