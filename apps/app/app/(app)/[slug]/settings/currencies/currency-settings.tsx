...
export function CurrencySettings() {
	...
	return (
		<div className="flex flex-col gap-6">
			<Card>
				<CardHeader>
					<CardTitle>Moeda de referência</CardTitle>
					<CardDescription>
				...
				<CardContent>
					<Field>
						...
						<FieldDescription>
							{canManage
								? "Mudar isso reconverte todo negócio pelas taxas de hoje. Números já reportados vão mudar."
								: "Só um proprietário ou administrador pode mudar como o dinheiro é reportado."}
						</FieldDescription>
					</Field>
				</CardContent>
			</Card>
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Quantos {reportingCurrency} uma unidade de cada moeda compra.
						Buscadas diariamente em open.er-api.com; uma taxa que você inserir
						aqui prevalece.
					</CardDescription>
					<CardAction>
						<Button
					...
					<form
						...
						onSubmit={(event) => {
							...
							if (!Number.isFinite(rate) || rate <= 0) {
								toast.error("A taxa precisa ser um número maior que zero.");
								return;
							}
				...
				{rates.length === 0 ? (
					<CardTableEmpty>
						Nenhuma taxa ainda. Atualize para buscá-las, ou insira uma
						manualmente.
					</CardTableEmpty>
				) : (
					<SimpleTable variant="embedded" columns={RATE_COLUMNS}>
						{rates.map((rate) => (
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						{unconverted.count === 0
							? "Todo negócio com um valor pode ser convertido para a moeda de referência."
							: `${formatCount(unconverted.count, "negócio")} não ${unconverted.count === 1 ? "pode" : "podem"} ser convertido${unconverted.count === 1 ? "" : "s"}, então fica${unconverted.count === 1 ? "" : "m"} de fora de todo total.`}
						{refreshedAt ? (
							<>
								{" "}
								Taxas buscadas pela última vez{" "}
				...
				{inUse.length === 0 ? (
					...
					<SimpleTable variant="embedded" columns={USAGE_COLUMNS}>
						{inUse.map((row) => (
