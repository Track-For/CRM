...
export function DealsBulkActions({
	...
	const setStage = useMutation(
		trpc.deals.bulkSetStage.mutationOptions({
			onSuccess: async (result) => {
				await cache.deal();
				reportBulk(result, (count) => `Movido: ${deals(count)}.`);
				setClosing(null);
				setReason("");
				onDone();
			},
	...
	const archive = useMutation(
		trpc.deals.bulkArchive.mutationOptions({
			onSuccess: async (result, variables) => {
				...
				reportBulk(result, (count) => `Arquivado: ${deals(count)}.`);
				onDone();
			},
			onError,
		}),
	...
	const restore = useMutation(
		trpc.deals.bulkRestore.mutationOptions({
			onSuccess: async (result) => {
				await cache.deal();
				reportBulk(result, (count) => `Restaurado: ${deals(count)}.`);
				onDone();
			},
			onError,
		}),
	...
	const purge = useMutation(
		trpc.deals.bulkPurge.mutationOptions({
			onSuccess: async (result, variables) => {
				await cache.removedMany({ kind: "deal", ids: variables.ids });
				reportBulk(result, (count) => `Excluído para sempre: ${deals(count)}.`);
				setConfirming(false);
				onDone();
			},
			onError,
		...
		return (
			<>
				...
				<BulkDeleteDialog
					...
					title={`Excluir ${deals(ids.length)} para sempre?`}
					description="Tudo o que está registrado neles — atividades, notas, os valores no seu pipeline — também é excluído. Isso não pode ser desfeito."
			...
			>
				<DialogContent>
					<DialogHeader>
						<DialogTitle>
							{closing === "CLOSED_LOST"
								? `Marcar ${deals(ids.length)} como perdido`
								: `Marcar ${deals(ids.length)} como desqualificado`}
						</DialogTitle>
						<DialogDescription>
							O mesmo motivo vale para todos eles, então mantenha o que têm em
							comum.
						</DialogDescription>
					</DialogHeader>
					...
					>
						<Field>
							...
							<Textarea
								...
								placeholder="Orçamento cancelado neste trimestre"
								rows={3}
							/>
						</Field>
					</form>
