...
async function SignIn({
	...
	if (!showSso && social.length === 0) {
		return (
			<>
				<AuthHeading
					title="Ainda não há como entrar"
					description="Este CRM não tem nenhum método de login configurado, então ninguém consegue entrar — nem você."
				/>

				<p className="text-center text-muted-foreground text-sm/5">
					Defina GOOGLE_CLIENT_ID e GOOGLE_CLIENT_SECRET — ou
					MICROSOFT_CLIENT_ID e MICROSOFT_CLIENT_SECRET — no arquivo .env raiz e
					reinicie. Seu próprio provedor de identidade pode ser adicionado em
					Configurações assim que alguém estiver conectado.
				</p>
			</>
		);
	}
