			...
			<div className="flex h-7 shrink-0 items-center justify-between pl-2">
				...
				<Button asChild variant="ghost" size="icon-xs">
					<Link
						...
						aria-label="Novo chat de agente"
						onClick={onNavigate}
					>
						<Icon icon={Add} />
					</Link>
			...
			>
				{groups.map((group) => (
					<div key={group.label}>
						...
						{group.items.map((conversation) => {
							...
							const title = conversation.title ?? "Chat sem título";
							return (
								<div
									key={conversation.id}
				...
				{groups.length === 0 ? (
					<p className="px-2 py-3 text-muted-foreground text-xs">
						Nenhum chat nos últimos 7 dias.
					</p>
				) : null}
...
function groupConversations(conversations: Conversation[], now: number) {
	...
	const labels: ChatDateGroup[] = ["Hoje", "Ontem", "Últimos 7 dias"];
