		...
		<div className="flex min-w-0 flex-col gap-4 sm:gap-6">
			<div className="flex min-h-7 items-center justify-start sm:justify-end">
				<select
					...
					aria-label="Filtrar resultados de execução"
					...
					<div className="flex min-w-0 items-stretch">
						...
						>
							...
							<span className="flex min-w-0 items-center justify-between gap-3 font-mono text-muted-foreground text-xs sm:shrink-0 sm:justify-start sm:gap-4">
								...
								<span>
									{run.actions.length}{" "}
									{run.actions.length === 1 ? "ação externa" : "ações externas"}
								</span>
								<Icon
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							O agente para onde estiver e a execução é registrada como
							cancelada. Tudo o que ele já fez — uma nota, uma tarefa, uma
							mensagem no Slack — permanece feito.
						</AlertDialogDescription>
					</AlertDialogHeader>
			...
			{visible.length === 0 ? (
				<p className="py-12 text-center text-muted-foreground text-sm">
					Nenhuma execução corresponde a este resultado.
				</p>
			) : null}
		</div>
	);
	...
	return (
		<div className="min-w-0 border-t">
			<div className="grid grid-cols-2 gap-x-4 gap-y-3 border-b bg-background px-4 py-3 sm:min-h-[58px] sm:grid-cols-4 sm:items-center sm:gap-0 sm:px-5 sm:py-2">
				...
				<RunMeta
					label="Iniciado por"
					value={run.initiatedBy?.name ?? "Agendador do Eve"}
				/>
			...
			<div>
				...
				{run.eventsTruncated ? (
					<div className="flex min-h-9 items-center border-t px-4 py-2 text-warning text-xs sm:px-5">
						Mostrando os primeiros {events.length} de {run.totalEvents} passos.
						Esta execução é longa demais para exibir por completo.
					</div>
				) : null}
			</div>
		</div>
			...
			<div className="min-w-0 overflow-hidden rounded-lg border bg-card">
				...
				{visible.length === 0 ? (
					<p className="px-5 py-12 text-center text-muted-foreground text-sm">
						Nenhuma alteração corresponde a este filtro.
					</p>
				) : null}
			</div>
		</div>
