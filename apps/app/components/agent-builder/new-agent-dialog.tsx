		...
		try {
			...
			toast.error(
				error instanceof InvalidInput
					? error.message
					: "Não foi possível repassar isto ao construtor.",
			);
		}
	};
		...
		<Dialog onOpenChange={setOpen} open={open}>
			...
			<DialogContent className="sm:max-w-(--container-sheet)">
				<DialogHeader>
					...
					<DialogDescription>
						Diga o que ele é e onde vive. O construtor escreve o resto. Você
						pode mudar tudo isso depois.
					</DialogDescription>
				</DialogHeader>
				...
				<div className="flex flex-col gap-4">
					<div className="flex flex-col gap-1.5">
						...
						<Input
							...
							placeholder="Resumo de preparação para renovação"
							value={name}
						/>
					</div>
					...
					<div className="flex flex-col gap-1.5">
						...
						<Textarea
							...
							placeholder="Uma semana antes de uma renovação, reúna o histórico da conta e publique um resumo curto para quem for responsável pelo negócio."
							rows={3}
							value={job}
						/>
					</div>
					...
					<div className="flex flex-col gap-1.5">
						...
						<p className="text-muted-foreground text-xs">
							{channel
								? channel.isMember
									? `A Comp AI já está em #${channel.name}.`
									: `A Comp AI ainda não está em #${channel.name}. Ela entra quando você criar isto.`
								: "Deixe em branco e o construtor vai perguntar."}
						</p>
					</div>
				...
				<DialogFooter className="items-center">
					...
					<Button disabled={!ready || create.isPending} onClick={hand}>
						{create.isPending ? "Repassando…" : "Repassar ao construtor"}
					</Button>
				</DialogFooter>
			</DialogContent>
		</Dialog>
