...
export function TeamAgentDetail({
	...
	const retryRun = useMutation(
		trpc.agents.retryRun.mutationOptions({
			onSuccess: async () => {
				await invalidate();
				toast.success("Execução enfileirada novamente.");
			},
			onError: (error) => toast.error(error.message),
		}),
	);
			...
			onSuccess: async (result) => {
				...
				toast.success(
					result.cancelled
						...
						: "Essa execução já tinha terminado.",
				);
			},
			onError: (error) => toast.error(error.message),
		}),
					...
					<div className="flex min-w-0 flex-col items-start gap-2 sm:items-end">
						...
						<div className="mt-1 flex flex-wrap gap-2">
							...
							<Button asChild variant="outline">
								<Link href={workspaceUrl("/chat")}>Abrir no chat</Link>
							</Button>
	...
	const deploy = useMutation(
		trpc.agents.deploy.mutationOptions({
			onSuccess: async () => {
				...
				toast.success("Agente publicado para a equipe.");
			},
			onError: (error) => toast.error(error.message),
		}),
	);
			...
			>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							Isso o remove da lista de agentes da equipe, desativa seus
							gatilhos e cancela execuções na fila. O histórico de execuções e
							ações permanece no log de auditoria. Uma execução já em andamento
							pode terminar.
						</AlertDialogDescription>
					</AlertDialogHeader>
	...
	if (!capabilities) {
		return (
			<p className="text-muted-foreground text-sm">
				Este agente ainda não tem uma versão publicada.
			</p>
		);
	}
	...
	return (
		<SaveBarViewport>
			<div className="flex flex-col gap-9">
				{deployed ? null : (
					<p className="text-muted-foreground text-sm">
						Isto é um rascunho. Publique para a equipe antes de mudar o que ele
						pode fazer.
					</p>
				)}
				<AgentCapabilities
