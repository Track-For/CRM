	...
	const revoke = useMutation(
		trpc.conversations.revokeShare.mutationOptions({
			onSuccess: async (_result, input) => {
				...
				await invalidate();
				toast.success("Link do chat revogado.");
			},
			onError: (error) => toast.error(error.message),
		}),
	);
					...
					{status.isPending ? (
						...
						<>
							...
							<ShareChoice
								...
								label="Qualquer pessoa na Comp AI com o link"
								...
								<div className="flex h-9 items-center gap-3 rounded-md border bg-background py-[3px] pr-[3px] pl-4">
									<span className="min-w-0 flex-1 truncate font-mono text-xs">
										{shareToken
											...
											: "Existe um link de somente leitura ativo"}
									</span>
									<Button
										variant="outline"
										size="sm"
							...
							<p className="text-muted-foreground text-xs">
								Compartilhar este chat não muda quem pode editar ou executar o
								agente.
							</p>
						</>
					)}
