...
const SUGGESTED_AGENTS = [
	"Informar o responsável do negócio antes de uma ligação de renovação",
	"Sinalizar negócios sem atividade há 14 dias",
	"Passar novos clientes de Vendas para Onboarding",
];

const FOLLOW_UPS = [
	{ label: "Reverificar Paula Marchetti", due: "14d", next: true },
	{ label: "Informar responsável antes da renovação", due: "2d", next: false },
	{ label: "Reenriquecer Northwind", due: "90d", next: false },
];
...
function EnrichmentCard() {
	return (
		<BentoCard className="gap-6">
			<CardHeading
				title="Os registros se preenchem sozinhos"
				body="Uma nova pessoa em uma thread vira um contato, e a empresa dela já chega com logo, setor e última atividade preenchidos."
			/>
...
function FollowUpCard() {
	return (
		<BentoCard className="min-w-0 grow gap-5">
			<CardTitle>Ele agenda os próprios follow-ups</CardTitle>
			...
			<div className="flex flex-col gap-2 pt-1">
				...
				<p className="text-[13px]/[21px] text-muted-foreground">
					Um agente que não consegue dizer por que vai voltar em catorze dias
					não tem um motivo, tem um padrão.
				</p>
			</div>
		</BentoCard>
	);
