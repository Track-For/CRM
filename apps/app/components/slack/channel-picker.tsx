						...
						<span className="relative flex w-20 shrink-0 items-center justify-end">
							{selected || (value === null && channel.isMember) ? (
								...
								<Button
									...
									variant="outline"
								>
									{channel.inviteRequestedAt ? "Pedir de novo" : "Solicitar"}
								</Button>
...
function describe(channel: PickerChannel, canInviteItself: boolean): string {
	...
	if (channel.isMember) return `A Comp AI está no canal${people}`;
	if (!channel.classified) return `Ainda não lido do Slack${people}`;
	if (!channel.isPrivate) return `A Comp AI pode entrar neste${people}`;
	if (canInviteItself) return `Privado. A Comp AI entra como você${people}`;
	if (channel.inviteRequestedAt)
		return `Privado. Aguardando um convite${people}`;
	return `Privado. Alguém de dentro precisa convidar a Comp AI${people}`;
}
