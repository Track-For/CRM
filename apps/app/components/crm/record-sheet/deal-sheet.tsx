...
function currencyOptions(currency: string) {
	...
	return [
		{ value: currency, label: `${currency} — não é mais suportada` },
		...CURRENCY_OPTIONS,
	];
}
		...
		<DetailSheetProperty label={`Em ${deal.reportingCurrency}`}>
			{deal.baseAmountCents === null ? (
				<span className="text-muted-foreground">
					Sem taxa de {currency} — fora dos totais
				</span>
			) : (
				...
				deal ? (
					<>
						...
						<RecordActions
							...
							name={deal.name}
							consequence={`O histórico de etapas, notas e conversas com o agente também são excluídos. ${deal.company.name} e quem está vinculado a ele continuam no CRM.`}
			...
			stats={
				deal ? (
					<DetailSheetStats>
						...
						<DetailSheetStat label="Previsão de fechamento">
						...
						<DetailSheetStat label="Nesta etapa">
		...
		<DetailSheetBody>
			...
			<DetailSheetSection title="Descrição">
				<InlineTextArea
					...
					placeholder={`O que a ${deal.company.name} está comprando, por que agora, e o que está no caminho.`}
			...
			<DetailSheetProperties>
				...
				<DetailSheetProperty label="Envolvidos" wide>
					{deal.contacts.length === 0 ? (
						<span className="text-muted-foreground">
							Ninguém da {deal.company.name} está vinculado ainda.
						</span>
					) : (
			...
			<>
				...
				{adding ? null : (
					<DetailSheetEmpty
						...
						description={`Ninguém da ${deal.company.name} está vinculado ainda. Adicione as pessoas para quem você está vendendo e fica claro quem contatar.`}
						action={
