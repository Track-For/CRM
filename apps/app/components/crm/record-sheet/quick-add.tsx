	...
	const create = useMutation(
		trpc.contacts.create.mutationOptions({
			onSuccess: async (contact) => {
				...
				toast.success(`${contact.firstName} adicionado.`);
				onDone();
			},
...
export function AttachDealContact({
	...
	const placeholder = options.isPending
		? "Carregando…"
		: nobody
			? `Todo mundo em ${companyName} já está no negócio`
			: "Escolha alguém";
...
export function QuickAddDeal({
	...
	const submit = () => {
		if (!owner) {
			toast.error(
				"Não foi possível determinar quem deve ser o responsável por este negócio.",
			);
			return;
		}
	...
	return (
		...
		>
			...
			<Field>
				<FieldLabel htmlFor={closeId}>Previsão de fechamento</FieldLabel>
				<DatePicker
					id={closeId}
					value={closeDate}
