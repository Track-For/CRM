				...
				contact ? (
					<>
						{contact.isPrimaryContact ? (
							<StatusIndicator
								tone="success"
								label={`Contato principal na ${contact.company?.name ?? "empresa"}`}
							/>
						) : null}
				...
				contact ? (
					<>
						...
						<RecordActions
							...
							consequence={`As notas dele, as conversas com o agente e tudo que o agente encontrou também são excluídos; e-mails e reuniões continuam registrados na empresa.${contact.email ? ` A sincronização não vai trazer ${contact.email} de volta — só adicionando manualmente.` : ""}`}
			...
			>
				<DetailSheetProperties>
					...
					<InlineField
						label="Cargo"
						value={contact.title}
						placeholder="Diretor de Segurança"
					...
					<InlineCompanyField
						...
						none={{ value: NONE, label: "Sem empresa" }}
						onSave={(companyId) =>
...
function Background({ brief }: { brief: NonNullable<Contact["brief"]> }) {
	...
	const lines = [
		...
		{ label: "Área", value: sections.function },
...
function WeKnowThem({
	...
	return (
		<DetailSheetSection title="O que já sabemos">
			<DetailSheetProperties>
				{emails > 0 ? (
				...
				{colleagues.length > 0 ? (
					<DetailSheetProperty label="Também aqui" wide>
...
function ContactDeals({ contact }: { contact: Contact }) {
	...
	if (contact.deals.length === 0) {
		return (
			<DetailSheetEmpty
				icon={Partnership}
				title="Sem negócios ainda"
				description={`${contactName(contact)} ainda não está vinculado a nenhuma venda. Negócios são abertos na empresa, e depois as pessoas são adicionadas a eles.`}
			/>
		);
	}
